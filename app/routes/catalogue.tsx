  ...
  return (
    <Layout title="Company Catalogue" subtitle="Manage your products, add new items manually, or import from Excel/CSV.">
      <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 32 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 20, flexWrap: "wrap" }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 16, flex: 1, maxWidth: 600 }}>
            <div style={{ position: "relative", flex: 1 }}>
              <Search size={18} color="var(--ui-text-muted)" style={{ position: "absolute", left: 16, top: "50%", transform: "translateY(-50%)" }} />
              <input 
                type="text" 
                ...
                style={{
                  ...
                  color: "var(--ui-text-primary)", outline: "none", fontSize: 14, boxSizing: "border-box"
                }}
              />
            </div>
            <div style={{ display: "flex", background: "var(--ui-bg-input)", padding: 4, borderRadius: 12, border: "1px solid var(--ui-border-input)" }}>
              <button onClick={() => setViewMode("grid")} style={{ padding: 8, borderRadius: 8, border: "none", background: viewMode === "grid" ? "var(--ui-bg-card)" : "transparent", cursor: "pointer" }}><LayoutGrid size={18} color={viewMode === "grid" ? "var(--ui-text-primary)" : "var(--ui-text-muted)"} /></button>
              <button onClick={() => setViewMode("list")} style={{ padding: 8, borderRadius: 8, border: "none", background: viewMode === "list" ? "var(--ui-bg-card)" : "transparent", cursor: "pointer" }}><List size={18} color={viewMode === "list" ? "var(--ui-text-primary)" : "var(--ui-text-muted)"} /></button>
            </div>
          </div>
        </div>
        ...
        {/* Bulk Import Section */}
        {!showForm && (
          <div className="glass-panel" style={{ padding: 24, borderRadius: 24, background: "var(--ui-bg-card)", border: "1px solid var(--ui-border)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 16 }}>
              <div>
                <h3 style={{ margin: 0, fontSize: 16, fontWeight: 800, color: "var(--ui-text-primary)" }}>Bulk Import</h3>
                <p style={{ margin: "4px 0 0", fontSize: 13, color: "var(--ui-text-muted)" }}>Upload your catalogue via Excel or CSV file.</p>
              </div>
              <form onSubmit={handleImportSubmit} style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                ...
                <label htmlFor="csv-upload" style={{
                  ...
                  background: "var(--ui-bg-input)", border: "1px solid var(--ui-border-input)",
                  borderRadius: 12, cursor: "pointer", fontSize: 13, color: file ? "var(--ui-text-primary)" : "var(--ui-text-muted)"
                }}>
            ...
            {result && (
              <div style={{ marginTop: 16, padding: "12px 16px", background: "rgba(34,197,94,0.08)", border: "1px solid rgba(34,197,94,0.2)", borderRadius: 12, color: "var(--huntr-green)", fontSize: 13 }}>
        ...
        <div style={{ minHeight: 400 }}>
          {itemsLoading ? (
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 12, padding: "100px 0" }}>
              <Loader2 className="animate-spin" color="var(--huntr-orange)" size={32} />
              <span style={{ fontSize: 14, color: "var(--ui-text-muted)" }}>Fetching your products...</span>
            </div>
          ) : filteredItems.length === 0 ? (
            <div style={{ textAlign: "center", padding: "100px 0", background: "var(--ui-bg-input)", borderRadius: 24, border: "1px dashed var(--ui-border-input)" }}>
              <Package size={48} color="var(--ui-text-muted)" style={{ marginBottom: 16, opacity: 0.3 }} />
              <h3 style={{ color: "var(--ui-text-primary)", margin: 0 }}>No products found</h3>
              <p style={{ color: "var(--ui-text-muted)", marginTop: 8 }}>{searchTerm ? "Try another search term" : "Start by adding your first product"}</p>
            </div>
          ) : viewMode === "grid" ? (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(300px, 1fr))", gap: 20 }}>
              {filteredItems.map(item => (
                <div key={item.id} className="glass-panel" style={{ padding: 24, display: "flex", flexDirection: "column", gap: 16, transition: "all 0.2s", background: "var(--ui-bg-card)", border: "1px solid var(--ui-border)" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    <div style={{ fontSize: 10, fontWeight: 800, color: "var(--ui-text-brand)", background: "var(--ui-bg-badge)", padding: "4px 10px", borderRadius: 6, letterSpacing: "0.05em" }}>
                      {item.item_code}
                    </div>
                  </div>
                  <div>
                    <h4 style={{ fontSize: 16, fontWeight: 800, color: "var(--ui-text-primary)", margin: 0 }}>{item.name}</h4>
                    <div style={{ fontSize: 12, color: "var(--ui-text-brand)", marginTop: 4 }}>{item.category || "General"}</div>
            ...
            <div className="glass-panel" style={{ overflow: "hidden", borderRadius: 24, background: "var(--ui-bg-card)", border: "1px solid var(--ui-border)" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", textAlign: "left" }}>
                <thead>
                  <tr style={{ background: "var(--ui-bg-input)", borderBottom: "1px solid var(--ui-border)" }}>
                    <th style={{ padding: "16px 24px", color: "var(--ui-text-muted)", fontSize: 11, fontWeight: 800, textTransform: "uppercase" }}>Item Info</th>
                    <th style={{ padding: "16px 24px", color: "var(--ui-text-muted)", fontSize: 11, fontWeight: 800, textTransform: "uppercase" }}>Category</th>
                    <th style={{ padding: "16px 24px", color: "var(--ui-text-muted)", fontSize: 11, fontWeight: 800, textTransform: "uppercase" }}>UOM</th>
                    <th style={{ padding: "16px 24px", color: "var(--ui-text-muted)", fontSize: 11, fontWeight: 800, textTransform: "uppercase", textAlign: "right" }}>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {filteredItems.map(item => (
                    <tr key={item.id} style={{ borderBottom: "1px solid var(--ui-border-subtle)", transition: "all 0.2s" }}>
                      <td style={{ padding: "20px 24px" }}>
                        <div style={{ color: "var(--ui-text-primary)", fontWeight: 700, fontSize: 14 }}>{item.name}</div>
                        <div style={{ color: "var(--ui-text-muted)", fontSize: 11, marginTop: 2, fontFamily: "monospace" }}>{item.item_code}</div>
                      </td>
                      <td style={{ padding: "20px 24px" }}>
                        <span style={{ color: "var(--ui-text-brand)", fontSize: 13 }}>{item.category || "General"}</span>
                      </td>
                      <td style={{ padding: "20px 24px", color: "var(--ui-text-secondary)", fontSize: 13 }}>{item.uom}</td>
...
const lbl: React.CSSProperties = { fontSize: 12, color: "var(--ui-text-muted)", fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.05em" };
const inputStyle: React.CSSProperties = {
  background: "var(--ui-bg-input)", border: "1px solid var(--ui-border-input)",
  borderRadius: 12, padding: "12px 16px", fontSize: 14, color: "var(--ui-text-primary)",
...
const primaryBtn: React.CSSProperties = {
  ...
  border: "none", background: "var(--huntr-gradient)", color: "#fff",
